...
var AddBoardingstop = React.createClass({
	...
	render: function() {
		...
		return (
			<Modal show={this.props.parent.state.showAddModal}>
				<Modal.Header>
					<Modal.Title>Add Boardingstop</Modal.Title>
				</Modal.Header>
				<Modal.Body>
					<form>
						<FormGroup>
							<ControlLabel>Boardingstop Name</ControlLabel>
							<FormControl
								type="text"
								placeholder="Enter Boardingstop"
								value={this.state.addObject.name}
								onChange={this.onAddBoardingstopNameChange} />
							<br />
						</FormGroup>
					</form>						
				</Modal.Body>
	...
	//Input changes
	onAddBoardingstopNameChange: function(event) {
	...
	onAddBtnClicked: function() {
		...
		//Save department
		axios.post('http://172.16.1.129:8080/boardingstops/', this.state.addObject)
...
export default AddBoardingstop;